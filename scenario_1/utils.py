...
def get_simple_data_loader():
    ...
    train_ls = [train_acc_meter, train_grav, train_gyro, train_lin_acc]

    train_tens = torch.cat(train_ls, dim=2)
    """
    change if activate when 2D or 1D
    """
    #train_tens = train_tens.view(2284,1, 12, 800)
    train_tens = train_tens.view(-1, 12, 800)
...
def get_simple_eval_loader():
    ...
    # eval_jin_acc = np.load('../../dataset/testing/trainJinsAccelerometer.npy')
    # eval_jin_gyro = np.load('../../dataset/testing/trainJinsGyroscope.npy')
    eval_lin_acc = torch.Tensor(np.load('../../dataset/testing/testLinearAcceleration.npy'))
    # eval_mag_meter = np.load('../../dataset/testing/trainMagnetometer.npy')
    ...
    # concat all training Tensors:
    eval_ls = [eval_acc_meter, eval_grav, eval_gyro, eval_lin_acc]
    ...
    """
    # eval_tens = eval_tens.view(-1, 1, 12, 800)
    eval_tens = eval_tens.view(-1, 12, 800)
